...
fn create_account() {
    ...
    let session = ds.find_or_create_account_via_session(&sc, true, false, false).expect("Should create account");
    ...
    let session2 = ds.find_or_create_account_via_session(&sc, true, false, false).expect("Should return account");
...
fn create_bobo_account(ds: &DataStore) -> sessionsrv::Session {
    ...
    sc.set_name(String::from("Bobo T. Clown"));
    sc.set_provider(sessionsrv::OAuthProvider::GitHub);
    ds.find_or_create_account_via_session(&sc, true, false, false).expect("Should create account")
}
...
fn get_account() {
    ...
    let bobo2 = ds.get_account(&ag).expect("Should run without error").expect("Bobo should exist");
...
fn get_account_by_id() {
    ...
    let bobo2 =
        ds.get_account_by_id(&ag).expect("Should run without error").expect("Bobo should exist");
...
fn get_session() {
    ...
    let session =
        ds.get_session(&sg).expect("Should run without error").expect("Session should exist");
